...
if ($bloco && isset($_GET['sala']) && $sala === 'Quadra' && !isset($_GET['pagina'])) {
    ...
    for ($i = 1; $i <= $grupos; $i++) {
        ...
        $fim = min($i * 8, $total);
        echo "<div>
                <a href='?bloco=" . urlencode($bloco) . "&sala=Quadra&pagina=$i'><button>Projetos " . $inicio . "-" . $fim . "</button></a>
            </div>";
    }
    echo "</div>";
...
if ($bloco && $sala) {
    ...
    $result = $stmt->get_result();
    if (!$result) {
        die("Erro ao obter resultado: " . $mysqli->error);
    }
    ...
    if ($result->num_rows === 0) {
        ...
        echo "<div class='BTNLocais'>";
        echo "<h1>Projetos da Sala " . htmlspecialchars($sala, ENT_QUOTES, 'UTF-8') . " (Bloco " . htmlspecialchars($bloco, ENT_QUOTES, 'UTF-8') . ")</h1>";
        while ($row = $result->fetch_assoc()) {
